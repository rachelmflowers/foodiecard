<?php include_once("../layouts/partials/header.php"); ?>
<main class="content">
    <div class="container">

        <section id="steps" class="steps-container">
            <h1 class=>How it works</h1>

            <div id="step-1" class="step">
                <div class="step-image">
                    <img src="" alt="" />
                </div>

                <div class="step-number active">
                    <p class="emphasized">Step</p>
                    <span>1</span>
                </div>

                <div class="step-content">
                    <h2>Commodo Cras Sit</h2>

                    <p>Maecenas sed diam eget risus varius blandit sit amet non magna. Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor.</p>

                    <a href="" class="btn btn-primary">Buy Foodie Card</a>
                </div>
            </div>

            <div id="step-2" class="step">
                <div class="step-content">
                    <h2>Commodo Cras Sit</h2>

                    <p>Maecenas sed diam eget risus varius blandit sit amet non magna. Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor.</p>

                    <a href="" class="btn btn-primary">Buy Foodie Card</a>
                </div>

                <span class="step-number">
                    <p class="emphasized">Step</p>
                    <span>2</span>
                </span>

                <div class="step-image">
                    <img src="" alt="" />
                </div>
            </div>

            <div id="step-3" class="step">
                <div class="step-image">
                    <img src="" alt="" />
                </div>

                <span class="step-number">
                    <p class="emphasized">Step</p>
                    <span>3</span>
                </span>

                <div class="step-content">
                    <h2>Commodo Cras Sit</h2>

                    <p>Maecenas sed diam eget risus varius blandit sit amet non magna. Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor.</p>

                    <a href="" class="btn btn-primary">Buy Foodie Card</a>
                </div>
            </div>
        </section>

        <section id="faqs" class="bg-full-gray">
            <h2 class="h1">Frequently Asked Questions</h2>

            <div class="accordion" id="faqAccordion">
                <div class="faq">
                    <div class="faq-header" id="question1">
                        <h5>
                            <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#answer1" aria-expanded="true" aria-controls="answer1">
                                <svg class="icon">
                                    <use xlink:href="/node_modules/feather-icons/dist/feather-sprite.svg#chevron-down" />
                                </svg>
                                Integer posuere erat a ante venenatis dapibus posuere velit aliquet.
                            </button>
                        </h5>
                    </div>

                    <div id="answer1" class="collapse show" aria-labelledby="question1" data-parent="#faqAccordion">
                        <div class="faq-body">
                            Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 
                        </div>
                    </div>
                </div>

                <div class="faq">
                    <div class="faq-header" id="question3">
                        <h5>
                            <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#answer2" aria-expanded="false" aria-controls="answer2">
                                <svg class="icon">
                                    <use xlink:href="/node_modules/feather-icons/dist/feather-sprite.svg#chevron-down" />
                                </svg>
                                Integer posuere erat a ante venenatis dapibus posuere velit aliquet.
                            </button>
                        </h5>
                    </div>

                    <div id="answer2" class="collapse" aria-labelledby="question2" data-parent="#faqAccordion">
                        <div class="faq-body">
                            Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 
                        </div>
                    </div>
                </div>

                <div class="faq">
                    <div class="faq-header" id="question3">
                        <h5>
                            <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#answer3" aria-expanded="false" aria-controls="answer3">
                                <svg class="icon">
                                    <use xlink:href="/node_modules/feather-icons/dist/feather-sprite.svg#chevron-down" />
                                </svg>
                                Integer posuere erat a ante venenatis dapibus posuere velit aliquet.
                            </button>
                        </h5>
                    </div>

                    <div id="answer3" class="collapse" aria-labelledby="question3" data-parent="#faqAccordion">
                        <div class="faq-body">
                            Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 
                        </div>
                    </div>
                </div>

                <div class="faq">
                    <div class="faq-header" id="question4">
                        <h5>
                            <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#answer4" aria-expanded="false" aria-controls="answer4">
                                <svg class="icon">
                                    <use xlink:href="/node_modules/feather-icons/dist/feather-sprite.svg#chevron-down" />
                                </svg>
                                Integer posuere erat a ante venenatis dapibus posuere velit aliquet.
                            </button>
                        </h5>
                    </div>

                    <div id="answer4" class="collapse" aria-labelledby="question4" data-parent="#faqAccordion">
                        <div class="faq-body">
                            Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 
                        </div>
                    </div>
                </div>

                <div class="faq">
                    <div class="faq-header" id="question5">
                        <h5>
                            <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#answer5" aria-expanded="false" aria-controls="answer5">
                                <svg class="icon">
                                    <use xlink:href="/node_modules/feather-icons/dist/feather-sprite.svg#chevron-down" />
                                </svg>
                                Integer posuere erat a ante venenatis dapibus posuere velit aliquet.
                            </button>
                        </h5>
                    </div>

                    <div id="answer5" class="collapse" aria-labelledby="question5" data-parent="#faqAccordion">
                        <div class="faq-body">
                            Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 
                        </div>
                    </div>
                </div>

                <div class="faq">
                    <div class="faq-header" id="question6">
                        <h5>
                            <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#answer6" aria-expanded="false" aria-controls="answer6">
                                <svg class="icon">
                                    <use xlink:href="/node_modules/feather-icons/dist/feather-sprite.svg#chevron-down" />
                                </svg>
                                Integer posuere erat a ante venenatis dapibus posuere velit aliquet.
                            </button>
                        </h5>
                    </div>

                    <div id="answer6" class="collapse" aria-labelledby="question6" data-parent="#faqAccordion">
                        <div class="faq-body">
                            Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 
                        </div>
                    </div>
                </div>

                <div class="faq">
                    <div class="faq-header" id="question7">
                        <h5>
                            <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#answer7" aria-expanded="false" aria-controls="answer7">
                                <svg class="icon">
                                    <use xlink:href="/node_modules/feather-icons/dist/feather-sprite.svg#chevron-down" />
                                </svg>
                                Integer posuere erat a ante venenatis dapibus posuere velit aliquet.
                            </button>
                        </h5>
                    </div>

                    <div id="answer7" class="collapse" aria-labelledby="question7" data-parent="#faqAccordion">
                        <div class="faq-body">
                            Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. 
                        </div>
                    </div>
                </div>



            </div>
        </section>
    </div>

    <?php include_once("../layouts/mailing-list.php"); ?>
</main>

<?php include_once("../layouts/partials/footer.php"); ?>